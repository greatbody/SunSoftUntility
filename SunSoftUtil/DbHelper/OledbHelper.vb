...
Namespace DbHelper
    Public Class OledbHelper
        ...
        Public Function Create(ByVal connString As String)
            '返回一个对象自身
            Return New OledbHelper(connString)
        End Function
        Public Shared Function Create() As OledbHelper
        ...
        Public Function ExcuteNonQuery(ByVal sqlComm As OleDbCommand) As Integer
            CreateConn(_connString)

            sqlComm.CommandText = _sqlBuilder.ToString
            Dim iAffected As Integer = sqlComm.ExecuteNonQuery()
        ...
        End Function
        Public Function ExecuteScalar(Of T)(ByVal sqlComm As OleDbCommand) As T
            '更新：改为在执行中打开连接，执行后关闭
            Dim obj As Object
            CreateConn(_connString)
        ...
        Public Shared Function From(ByVal sqlStr As String) As OledbHelper
            Return New OledbHelper(sqlStr)
        End Function

        Public Function FillDataTable(ByVal sqlComm As OleDbCommand) As DataTable
            '更新：改为在执行中打开连接，执行后关闭
            Dim dataTable As New DataTable("_sunsoft")

            sqlComm.CommandText = _sqlBuilder.ToString
            CreateConn(_connString)
            Using dbReader As OleDbDataReader = sqlComm.ExecuteReader
        ...
        Public Function FillDataSet(ByVal sqlComm As OleDbCommand) As DataSet
            '更新：改为在执行中打开连接，执行后关闭
            Dim dataSet As New DataSet
            sqlComm.CommandText = _sqlBuilder.ToString
            CreateConn(_connString)
            Dim dataAdpt As OleDbDataAdapter = New OleDbDataAdapter(sqlComm)
        ...
        '''后面的代码用于运算符重载相关的代码
        Public Shared Operator +(ByVal meDefault As OledbHelper, ByVal strp As String) As OledbHelper
            '完成:运算符重载
        ...
        End Sub
        Public Shared Operator +(ByVal meDefault As OledbHelper, ByVal strp As QueryParameter) As OledbHelper
